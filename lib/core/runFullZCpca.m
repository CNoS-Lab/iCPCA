function [rpt_obj, CPCA_struct, PCA_struct] = runFullZCpca(rpt_obj, params)
...
temp_dims = min([rank(Z), rank(G)]);
CPCA_struct_temp = runCPCA(Z,G,1,temp_dims,0);

if params.default_flag
    nComp = params.default_struct.nComp;
else
    plotScree(CPCA_struct_temp);
    nComp = input("Full Z CPCA - Enter the number of components: ");
end

CPCA_struct = runCPCA(Z,G,1,nComp,params.varimaxFlag);
CPCA_struct.varianceTable = getVarianceTable(CPCA_struct);
CPCA_struct.PCorr = corr(CPCA_struct.scores_NU_GH, G)';
CPCA_struct.nComp = nComp;

permutePCorrPCA = NaN([size(CPCA_struct.PCorr),params.n_bootstrap]);
permutePCorrCPCA = NaN([size(CPCA_struct.PCorr),params.n_bootstrap]);

parfor i = 1:params.n_bootstrap
    G_temp = G(randperm(size(G,1)),:); Z_temp = Z;
    temp_struct = runCPCA(Z_temp,G_temp,1,nComp,params.varimaxFlag);
    temp_struct.PCorr = corr(temp_struct.scores_NU_GH, G_temp)';
    permutePCorrCPCA(:,:,i) = temp_struct.PCorr;

    temp_struct = runCPCA(Z,1,1,nComp,params.varimaxFlag);
    temp_struct.PCorr = corr(temp_struct.scores_NU_GH, G_temp)';
    permutePCorrPCA(:,:,i) = temp_struct.PCorr;
end

CPCA_struct.predrel_cutoff = quantile(abs(permutePCorrCPCA(:)), 1-params.p_val);
CPCA_struct.RelPredLoadMask = abs(CPCA_struct.PCorr) >=  CPCA_struct.predrel_cutoff;

CPCA_struct.nullDist = permutePCorrCPCA;
CPCA_struct.params = params;

%% Perform PCA

PCA_struct = runCPCA(Z,1,1,nComp,params.varimaxFlag);
PCA_struct.varianceTable = getVarianceTable(PCA_struct);
PCA_struct.PCorr = corr(PCA_struct.scores_NU_GH, G)';
PCA_struct.predrel_cutoff = quantile(abs(permutePCorrPCA(:)), 1-params.p_val);
PCA_struct.RelPredLoadMask = abs(PCA_struct.PCorr) >=  PCA_struct.predrel_cutoff;

PCA_struct.nullDist = permutePCorrPCA;
PCA_struct.nComp = nComp;
PCA_struct.params = params;
...
for ii = 1:size(CPCA_struct.PCorr,1)
    for jj = 1:size(CPCA_struct.PCorr,2)
        PCA_struct.pVales(ii,jj) = mean(abs(permutePCorrPCA(:)) > abs(PCA_struct.PCorr(ii,jj)),"all");
        CPCA_struct.pVales(ii,jj) = mean(abs(permutePCorrCPCA(:)) > abs(CPCA_struct.PCorr(ii,jj)),"all");
    end
end
...
PCA_struct.Res = SelectCompLoadings(PCA_struct, params);
[rpt_obj_sub_pca, f, PCA_struct] = addPlots(rpt_obj_sub_pca, f, PCA_struct, params, titleStr);
...
CPCA_struct.Res = SelectCompLoadings(CPCA_struct, params);
[rpt_obj_sub_cpca, f, CPCA_struct] = addPlots(rpt_obj_sub_cpca, f, CPCA_struct, params, titleStr);
...
percent_var = cpca_struct.varianceTable.perVarEachS;
...
T = Text(sprintf('Chapter: %s \n', params.page_header)); add(rpt_obj, T);
H = Heading4(Text(sprintf('%s - Predictor loading threshold - p <= 0.005', titleStr)));
add(rpt_obj, H);
...
H = Heading4(Text(sprintf('%s - Predictor Loadings (Multiple comparison corrected p-value)',titleStr)));
add(rpt_obj, H);

nz = 10;
temp = mean(cpca_struct.PCorr, 3, 'omitnan');
n_plots = ceil(size(temp,1)/nz);
...
for ii = 1:n_plots
    ...
    fig = figure('visible','off');
    if ii == n_plots
        h = plotLoadings4(temp(nz*(ii-1)+1:end,:),xStr,params.Glabel(nz*(ii-1)+1:end),cpca_struct.pVales_corrected(nz*(ii-1)+1:end,:), [0.05,0.01,0.001], cpca_struct.predrel_cutoff);
    else
        h = plotLoadings4(temp(nz*(ii-1)+1:nz*ii,:),xStr,params.Glabel(nz*(ii-1)+1:nz*ii),cpca_struct.pVales_corrected(nz*(ii-1)+1:nz*ii,:), [0.05,0.01,0.001], cpca_struct.predrel_cutoff);
    end
    h.Colormap = flipud(cbrewer('div','RdBu',64,'spline'));
    h.CLim = [-0.3 0.3];
    if n_plots == 1
        title(sprintf('%s - Predictor Loadings (Multiple comparison corrected p-value)',titleStr));
    else
        title(sprintf('%s - Predictor Loadings (Multiple comparison corrected p-value) plot no. %d of %d ',titleStr,ii,n_plots));
    end
...
%% function addDominantCompLoadPlot
% 
%
%
%% Params:
...
if ~isempty(comp_rel)
    ...
    subplot(1,2,1);
    temp = sum(abs(cpca_struct.PCorr.*cpca_struct.RelPredLoadMask),1)./sum(cpca_struct.RelPredLoadMask,1);
    ...
    ylabel('Mean of reliable Pred. Load.');
    xlabel('Variable dominance rank #');

    subplot(1,2,2);
    temp = sum(abs(cpca_struct.PCorr.*cpca_struct.RelPredLoadMask_corrected),1)./sum(cpca_struct.RelPredLoadMask_corrected,1);
...
for comp = comp_rel
    ...
    subplot(1,2,1);
    temp = sum(abs(cpca_struct.PCorr(:,comp).*cpca_struct.RelPredLoadMask(:,comp)),1)./sum(cpca_struct.RelPredLoadMask(:,comp),1);
    ...
    subplot(1,2,2);
    temp = sum(abs(cpca_struct.PCorr(:,comp).*cpca_struct.RelPredLoadMask_corrected(:,comp)),1)./sum(cpca_struct.RelPredLoadMask_corrected(:,comp),1);
...
while Add_more_summary_flag && ~isempty(comp_rel)
    ...
    for c = 1:length(comp_rel_summ)
        ...
        for ii = 1:length(rel_pred_idx)
            ...
            cpca_struct.Summary(summ_no,c).PredLoadings(ii).Value = cpca_struct.PCorr(rel_pred_idx(ii),comp_rel_summ(c));
            ...
            cpca_struct.Summary(summ_no,c).PredLoadings(ii).PredLoadRelProp = NaN;
